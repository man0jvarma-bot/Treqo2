...
export default function KnowledgeHub() {
  ...
  return (
    <section className="py-24 px-[5%] bg-white text-slate-900 font-sans">
      <div className="max-w-7xl mx-auto">

        {/* REFINED LIGHT HEADER */}
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 bg-purple-50 px-5 py-2 rounded-full mb-6 border border-purple-100">
            <Sparkles size={14} className="text-purple-600" />
            <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-purple-700">Curated Learning Path</span>
          </div>

          <h2 className="text-4xl md:text-6xl font-bold tracking-tight mb-6">
            Inside the <span className="text-purple-600 italic font-serif">8 Course</span> Suite
          </h2>

          <p className="max-w-2xl mx-auto text-slate-500 text-base md:text-lg leading-relaxed font-medium">
            ...
            {courses.map((course) => (
              <button
                ...
                className={`flex items-center gap-4 p-4 rounded-[20px] border transition-all duration-300 text-left
                  ${active === course.id
                    ? "bg-purple-50/50 border-purple-100 shadow-[0_10px_30px_rgba(124,58,237,0.06)] scale-[1.02]"
                    : "bg-transparent border-transparent hover:bg-purple-50/30"}`}
              >
                <div className={`w-10 h-10 flex items-center justify-center rounded-2xl transition-all duration-500 ${active === course.id ? "bg-purple-600 text-white" : "bg-slate-100 text-slate-400"}`}>
                  {course.icon}
                </div>
                <div>
                  <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Module 0{course.id}</p>
                  <p className={`text-xs font-bold transition-colors ${active === course.id ? "text-slate-900" : "text-slate-400"}`}>
                    {course.title}
                  </p>
                </div>
              </button>
          ...
          <div className="flex-1 w-full min-w-0">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="bg-white rounded-[40px] p-8 md:p-14 shadow-[0_30px_70px_rgba(124,58,237,0.04)] border border-slate-100 min-h-[700px] flex flex-col"
              >
                <div className="flex flex-col xl:flex-row justify-between items-start gap-8 mb-12">
                  <div className="max-w-xl">
                    <div className="flex items-center gap-3 mb-4">
                      <span className="px-3 py-1 bg-purple-50 text-purple-700 text-[10px] font-bold uppercase rounded-md border border-purple-100">Course Syllabus</span>
                      <div className="w-1.5 h-1.5 rounded-full bg-purple-600 animate-pulse" />
                    </div>
                    <h3 className="text-4xl md:text-5xl font-bold mb-3 tracking-tight text-slate-900">{selected?.title}</h3>
                    <p className="text-purple-600 font-serif italic text-2xl mb-6">{selected?.heading}</p>
                    ...
                    <div className="p-6 bg-slate-50 rounded-[30px] border border-slate-100">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {selected?.tools.map((tool, i) => (
                          <span key={i} className="text-[10px] font-bold bg-white px-3 py-1.5 rounded-lg shadow-sm border border-slate-200 text-slate-700">{tool}</span>
                        ))}
                      </div>
                    </div>
                  </div>
                ...
                <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-8 mb-12 flex-1">
                  <div className="space-y-6">
                    <h4 className="text-[11px] font-bold uppercase tracking-[0.2em] text-slate-300 border-b pb-2">Core Curriculum</h4>
                  ...
                  <div className="space-y-6">
                    <h4 className="text-[11px] font-bold uppercase tracking-[0.2em] text-slate-300 border-b pb-2">Strategic Roadmap</h4>
                    <div className="space-y-4">
                      {selected?.roadmap.map((step, i) => (
                        <div key={i} className="flex items-center gap-4">
                          <span className="text-[10px] font-black text-purple-200">0{i + 1}</span>
                  ...
                  <div className="space-y-6">
                    <h4 className="text-[11px] font-bold uppercase tracking-[0.2em] text-slate-300 border-b pb-2">Primary Outcome</h4>
                    <div className="p-6 bg-purple-600 text-white rounded-[32px] shadow-lg shadow-purple-900/10 relative overflow-hidden group">
                      ...
                      <h4 className="text-[10px] font-bold uppercase tracking-[0.2em] opacity-60 mb-4 text-purple-100">Hands-on Project</h4>
                ...
                <div className="flex flex-col sm:flex-row gap-5 items-center pt-8 border-t border-slate-50">
                  <button className="w-full sm:w-auto px-10 py-4 bg-purple-600 text-white rounded-[18px] font-bold text-sm hover:shadow-xl hover:bg-purple-700 hover:-translate-y-1 transition-all flex items-center justify-center gap-3">
                    Start Learning Module <ArrowRight size={18} />
                  </button>
                  <button className="w-full sm:w-auto px-10 py-4 bg-transparent text-slate-400 border border-slate-200 rounded-[18px] font-bold text-sm hover:bg-slate-50 transition-all flex items-center justify-center gap-2">
                    <Download size={18} /> Syllabus (PDF)
                  </button>
                  <div className="hidden xl:flex items-center gap-2 ml-auto text-slate-300">
